    ...
    def parse(self, response):
        ...
        for i in range(pageCount):
            ...
            if selectors_feedlist:
                ...
                '''
                # 用re正则表达式去除空格
                title = selectors_feedlist.xpath('div/div[1]/h2/a/text()').extract_first()
                title = re.sub(r'\s', '', title)
                # 用strip()函数去除空格
                # title.strip()

                summary = selectors_feedlist.xpath('div/div[2]/text()').extract_first()
                summary = re.sub(r'\s', '', summary)
                # 用strip()函数去除空格
                # summary.strip()

                readCount = selectors_feedlist.xpath('div/dl/div[2]/dd[1]/a/span[2]/text()').extract_first()
                readCount = re.sub(r'\s', '', readCount)

                author = selectors_feedlist.xpath('div/dl/dd[1]/a/text()').extract_first()
                author = re.sub(r'\s', '', author)

                url = selectors_feedlist.xpath('div/div[1]/h2/a/@href').extract_first()
                re.sub(r'\s', '', url)

                date = selectors_feedlist.xpath('div/dl/dd[2]/text()').extract_first()
